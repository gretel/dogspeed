...
class MPU6886:
    def __init__(self,i2c,address=104,accel_fs=ACCEL_FS_SEL_2G,gyro_fs=GYRO_FS_SEL_250DPS,accel_sf=SF_M_S2,gyro_sf=SF_RAD_S,gyro_offset=(0,0,0)):
        ...
        if 25!=self.whoami:raise RuntimeError('MPU6886 not found in I2C bus.')
        self._register_char(_PWR_MGMT_1,128);utime.sleep_ms(100);self._register_char(_PWR_MGMT_1,1);self._accel_so=self._accel_fs(accel_fs);self._gyro_so=self._gyro_fs(gyro_fs);self._accel_sf=accel_sf;self._gyro_sf=gyro_sf;self._gyro_offset=gyro_offset
    @property
    ...
    @property
    def temperature(self):temp=self._register_short(_TEMP_OUT_H);return temp/_TEMP_SO+_TEMP_OFFSET
    @property
    def whoami(self):return self._register_char(_WHO_AM_I)
    def calibrate(self,count=256,delay=0):
        ox,oy,oz=0.0,0.0,0.0;self._gyro_offset=0.0,0.0,0.0;n=float(count)
    ...
    def _register_short(self,register,value=None,buf=bytearray(2)):
        if value is None:self.i2c.readfrom_mem_into(self.address,register,buf);return ustruct.unpack('>h',buf)[0]
        ustruct.pack_into('>h',buf,0,value);return self.i2c.writeto_mem(self.address,register,buf)
    ...
    def _register_char(self,register,value=None,buf=bytearray(1)):
        if value is None:self.i2c.readfrom_mem_into(self.address,register,buf);return buf[0]
